...
def generate_dungeon(
        ...
        for r in range(max_rooms):
            ...
            #RectangularRoom class makes rectangular easier to work with
...
def place_entites(
    ...
    for i in range(number_of_monsters):
        ...
        #Check if there is no entity on current cell and then randomly place a mob.
        if not any(entity.x == x and entity.y == y for entity in dungeon.entities):
            if random.random() <0.8:
                et.orc.spawn(dungeon,x,y)
            else:
                et.troll.spawn(dungeon,x,y)
